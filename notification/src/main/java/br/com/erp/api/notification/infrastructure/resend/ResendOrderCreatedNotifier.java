...
public class ResendOrderCreatedNotifier implements OrderCreatedNotifier {
    ...
    @Override
    public void notify(Order order, Customer customer, String whatsappUrl) throws Exception {
        Resend resend = new Resend(apiKey);
        CreateEmailOptions request = CreateEmailOptions.builder()
                .from(from)
                .to(adminEmail)
                .subject("Novo pedido #" + order.getId() + " — responda pelo WhatsApp")
                .html(buildHtmlBody(order, customer, whatsappUrl))
                .build();
        resend.emails().send(request);
    }

    private String buildHtmlBody(Order order, Customer customer, String whatsappUrl) {
        ...
        return EMAIL_TEMPLATE
                ...
                .replace("{whatsappUrl}", escapeAttr(whatsappUrl))
