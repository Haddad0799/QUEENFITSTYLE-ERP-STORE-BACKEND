...
public class ExpireOrderUseCase {
    ...
    public Order expireOrder(Order order, String actor) {
        ...
        // Avisa a cliente (e-mail) após o commit — efeito colateral desacoplado, tratado por
        // listener AFTER_COMMIT. Publicado só aqui (após a transição real), nunca no no-op de
        // idempotência acima, para não notificar um pedido que já estava EXPIRED.
        eventPublisher.publishEvent(new OrderExpiredEvent(order));
